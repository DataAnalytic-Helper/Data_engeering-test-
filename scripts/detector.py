...
def upload_snapshot(frame):
    log_event("☁️ Загрузка снимка в MinIO...")
    # Здесь должна быть твоя логика Minio client.put_object
                ...
                for i in range(len(ids)):
                    ...
                    if d_end < LINE_THRESHOLD and t_id in entry_times and t_id not in processed_ids:
                        ...
                        if duration > 0.3:
                            ...
                            if db_active:
                                try:
                                    cursor.execute("INSERT INTO user43.full_tracking_data (track_id, speed_km_h, detection_time) VALUES (%s, %s, NOW())", (t_id, float(speed)))
                                except:
                                    db_active = False
                    ...
                    for j in range(i + 1, len(active_objects)):
                        ...
                        if dist_m < CRITICAL_DIST_M:
                            ...
                            if pair not in last_incident_time or (now_ts - last_incident_time[pair]) > INCIDENT_COOLDOWN:
                                ...
                                if db_active:
                                    try:
                                        cursor.execute("INSERT INTO user43.dangerous_incidents (time, track_id1, track_id2, distance) VALUES (NOW(), %s, %s, %s)", (o1['id'], o2['id'], float(dist_m)))
                                    except Exception as e:
                                        print(f"❌ Ошибка инцидента: {e}")
                                last_incident_time[pair] = now_ts

            # Отрисовка ROI и интерфейса
